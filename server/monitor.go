...
func (s *Server) Connz(opts *ConnzOptions) (*Connz, error) {
	...
	for _, client := range openClients {
		...
		if len(client.subs) > 0 {
			if subsDet {
				ci.SubsDetail = make([]SubDetail, 0, len(client.subs))
				for _, sub := range client.subs {
					ci.SubsDetail = append(ci.SubsDetail, NewSubDetail(sub))
				}
			} else if subs {
	...
	for _, r := range s.routes {
		...
		if len(r.subs) > 0 {
			if opts.SubscriptionsDetail {
				ri.SubsDetail = make([]SubDetail, 0, len(r.subs))
				for _, sub := range r.subs {
					ri.SubsDetail = append(ri.SubsDetail, NewSubDetail(sub))
				}
...
func (s *Server) HandleRoutez(w http.ResponseWriter, r *http.Request) {
	subsDetail := strings.ToLower(r.URL.Query().Get("subs")) == "detail"
	subs := false
	if !subsDetail {
		var err error
		subs, err = decodeBool(w, r, "subs")
